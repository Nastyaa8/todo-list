...
render(){
    ...
    this.listElement.innerHTML = items.map(({id,title,isChecked})=> `
     <li class="todo-item" data-js-todo-item>
            <input
            type="checkbox" 
            ${isChecked ? 'checked': ''}
            ...
            <label for="${id}" class="todo-item-label" data-js-todo-item-label>${title}</label>
            <button class="todo-item_delete_button" type="button" aria-label="Delete" title="Delete" data-js-todo-item-delete-button>
...
<path d="M15 5L5 15M5 5L15 15" stroke="#757575" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        :isEmptyItems ? 'There are no thanks yet'
        :''
}
addItem(title){
